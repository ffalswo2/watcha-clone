...
function savePayment($userIdxInToken,$itemName) {
    try {
        $pdo = pdoSqlConnect();

        $pdo->beginTransaction();

        $query1 = "INSERT INTO purchased (userIdx,itemName) VALUES (?,?);";

        $st = $pdo->prepare($query1);
        $st->execute([$userIdxInToken,$itemName]);

        $query2 = "INSERT INTO membership (userIdx,membershipName) VALUES (?,?);";
...
function getLoginFlag($userIdxInToken,$deviceId){
    $pdo = pdoSqlConnect();
